...
public class CarsInGarageDAO implements DAO<CarsInGarage, Integer> {
    ...
    private final static String FINDCARSBYID = "SELECT id, model, tuition FROM Cars WHERE id = ?";
    private final static String FINDALLCARSINGARAGE = "SELECT c.id, c.model, c.tuition FROM Cars AS c JOIN Stores AS s ON c.id = s.carsId WHERE s.garageId = ?";
    ...
    public List<Cars> findAllCarsInGarage(int garageId) {
        ...
        try (PreparedStatement ps = ConnectionMariaDB.getConnection().prepareStatement(FINDALLCARSINGARAGE)) {
            ...
            while (rs.next()) {
                ...
                Cars cars = new Cars(id, model, tuition);
                carsList.add(cars);
            }
        } catch (SQLException e) {
    ...
    public Cars findCarsById(int carsId) {
        ...
        try (PreparedStatement ps = ConnectionMariaDB.getConnection().prepareStatement(FINDCARSBYID)) {
            ...
            if (rs.next()) {
                ...
                cars = new Cars(id, model, tuition);
            }
        } catch (SQLException e) {
