...
public class UserDAO implements DAO<User, String> {
    ...
    public User save(User user) {
        ...
        if (u == null) {
            try (PreparedStatement ps = ConnectionMariaDB.getConnection().prepareStatement(INSERT)) {
                ...
                if (user.getGarages() != null) {
                    for (Garage garage : user.getGarages()) {
    ...
    public User update(User user) {
        try (PreparedStatement ps = ConnectionMariaDB.getConnection().prepareStatement(UPDATE)) {
            ...
            if (user.getGarages() != null) {
                List<Garage> garagesBefore = GarageDAO.build().findByUser(user.getUsername());
                List<Garage> garagesAfter = user.getGarages();

                List<Garage> garagesToBeDeleted = new ArrayList<>(garagesBefore);
                garagesToBeDeleted.removeAll(garagesAfter);

                for (Garage garage : garagesToBeDeleted) {
                    GarageDAO.build().delete(garage);
                }
                for (Garage garage : garagesAfter) {
                    GarageDAO.build().save(garage);
                }
            }
