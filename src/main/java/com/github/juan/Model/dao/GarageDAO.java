...
public class GarageDAO implements DAO<Garage, Integer> {
    private final static String INSERT="INSERT INTO Garage (location, user_username) VALUES (?, ?);";
    private final static String UPDATE = "UPDATE Garage SET location = ? WHERE id = ?";
    private final static String FINDALL = "SELECT g.id, g.location FROM Garage AS g";
    private final static String FINDBYID = "SELECT g.id, g.location FROM Garage AS g WHERE g.id = ?";
    ...
    public Garage saveGarage(Garage garage, User currentUser) {
        ...
        Garage g = findById(garage.getId());
        if (g == null) {
